...
const SuperAdminDashboard = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [met, users, depts, blockers, delays, contribs, leads, tasks] = await Promise.all([
        apiService('/dashboard/overview'), apiService('/users?status=PENDING'), apiService('/departments'),
      ...
      setMetrics(met); setPendingUsers(users); setDepartments(depts); setBlockerTasks(blockers);
  ...
  const handleApproveSubmit = async (e) => {
    ...
    try {
      await apiService('/assign-role', { method: 'PATCH', body: JSON.stringify({ userId: selectedUser._id, role: approvalRole }) });
      if ((approvalRole === 'CONTRIBUTOR' || approvalRole === 'TEAM_LEAD') && approvalDept) {
        await apiService('/assign-department', { method: 'PATCH', body: JSON.stringify({ userId: selectedUser._id, departmentId: approvalDept }) });
      }
      setApproveModalOpen(false); setSelectedUser(null); loadData();
  ...
  return (
    <div className="tc-fade-in">
      <div className="tc-page-header">
        <div>
          <h1 className="tc-heading-xl">Organization Dashboard</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 24 }}>
        <div className="tc-stat-card blue" onClick={() => navigate('/users')}>
          ...
          <div className="tc-stat-card-desc">+2 registered recently</div>
        </div>
        <div className={`tc-stat-card ${metrics?.pendingApprovalsCount > 0 ? 'orange' : 'gray'}`} onClick={() => navigate('/users')}>
          <div className="tc-stat-card-label">Pending Approvals</div>
          <div className="tc-stat-card-value" style={{ color: metrics?.pendingApprovalsCount > 0 ? '#f97316' : undefined }}>{metrics?.pendingApprovalsCount || 0}</div>
          <div className="tc-stat-card-desc">Action required</div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '40% 30% 30%', gap: 16, marginBottom: 24 }}>
        ...
        <div className="tc-card" style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ padding: '16px 24px' }}>
            <button className="tc-btn tc-btn-outline" style={{ width: '100%', justifyContent: 'center' }} onClick={() => navigate('/admin/departments')}>Manage Departments</button>
          </div>
        </div>
        <div className="tc-card">
          <div className="tc-card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      ...
      <div className="tc-grid-3" style={{ marginBottom: 24 }}>
        <div className="tc-card">
          <div className="tc-card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h2 className="tc-heading-sm">Pending Approvals ({pendingUsers.length})</h2>
            <button className="tc-btn tc-btn-secondary tc-btn-xs" onClick={() => navigate('/users')}>View All</button>
          </div>
          <div className="tc-card-body" style={{ padding: '0 24px 24px', display: 'flex', flexDirection: 'column', gap: 12 }}>
            {pendingUsers.length === 0 ? <div style={{ textAlign: 'center', padding: '20px 0', color: 'var(--color-text-secondary)' }}>All caught up!</div> :
              pendingUsers.slice(0, 3).map(u => (
                <div key={u._id} style={{ padding: 12, background: 'var(--color-bg-subtle)', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div><div className="tc-heading-sm">{u.name}</div><div className="tc-caption">{u.designation}</div></div>
                  <button className="tc-btn tc-btn-primary tc-btn-sm" onClick={() => { setSelectedUser(u); setApprovalRole('CONTRIBUTOR'); setApproveModalOpen(true); }}>Review</button>
                </div>
              ))
            }
          </div>
        ...
        <div className="tc-modal-overlay">
          <div className="tc-modal" style={{ maxWidth: 450 }}>
            <div className="tc-modal-header">
              <h2 className="tc-heading-lg">Approve Employee</h2>
            </div>
            <form onSubmit={handleApproveSubmit}>
              <div style={{ marginBottom: 16 }}>
                <div className="tc-heading-md">{selectedUser?.name}</div>
                <div className="tc-caption">{selectedUser?.email} • {selectedUser?.designation}</div>
              </div>
              <div className="tc-form-group" style={{ marginBottom: 16 }}>
                <label className="tc-form-label">Assign Role</label>
                <select className="tc-form-select" value={approvalRole} onChange={e => setApprovalRole(e.target.value)} required>
                  <option value="CONTRIBUTOR">Contributor</option><option value="TEAM_LEAD">Team Lead</option>
                  <option value="MANAGEMENT">Management</option><option value="SUPER_ADMIN">Super Admin</option>
                </select>
              </div>
              {(approvalRole === 'CONTRIBUTOR' || approvalRole === 'TEAM_LEAD') && (
                <div className="tc-form-group">
                  <label className="tc-form-label">Assign Department</label>
                  <select className="tc-form-select" value={approvalDept} onChange={e => setApprovalDept(e.target.value)} required>
              ...
              <div className="tc-modal-footer">
                <button type="button" className="tc-btn tc-btn-secondary" onClick={() => setApproveModalOpen(false)}>Cancel</button>
                <button type="submit" className="tc-btn tc-btn-primary">Approve Access</button>
              </div>
            </form>
          </div>
        </div>
